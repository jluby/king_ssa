...
proposals <- read.csv("data/proposal_scoring.csv", stringsAsFactors=F)
...
<<hmd-qx-time,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
qx_time <- ggplot(lt_us[lt_us$Age<110,], aes(x=Year, y=log(qx), color=Age, group=Age)) + geom_line() + scale_x_continuous("Year", limits=c(1980,2010)) + scale_y_continuous("Conditional Probability of Death (Log Scale)", breaks=log(c(0.0005,0.005, 0.05, 0.5)), label=paste(c(0.0005,0.005, 0.05, 0.5)*100,"%", sep="")) + facet_wrap(~Sex) + theme(strip.background = element_rect(fill="snow1")) + scale_color_gradientn("Age",colours=rainbow(7), breaks=seq(0,110,20)) + theme(text=element_text(size=15, color="black"))

ggsave(filename='figures/graphics-hmd-qx-time-1.pdf', plot=qx_time, height=5, width=8)
@
\end{figure}
...
<<hmd-qx-age,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
qx_age <- ggplot(lt_us[lt_us$Year>=1980 & lt_us$Age<110,], aes(x=Age, y=log(qx), color=Year, group=Year)) + geom_line()  + scale_x_continuous("Age") + scale_y_continuous("Conditional Probability of Death (Log Scale)",breaks=log(c(0.0005,0.005, 0.05, 0.5)), label=paste(c(0.0005,0.005, 0.05, 0.5)*100,"%", sep="")) + facet_wrap(~Sex) + theme(strip.background = element_rect(fill="snow1")) + scale_color_gradientn("Year",colours=rainbow(7), breaks=seq(1980,2010,10)) + theme(text=element_text(size=15, color="black"))

ggsave(filename='figures/graphics-hmd-qx-age-1.pdf', plot=qx_age, height=5, width=8)
@
\end{figure}
...
<<ageprofiles,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
...
ggsave(filename='figures/graphics-ageprofiles-1.pdf', plot=qx_age_countries, height=6, width=8)
@
\end{figure}
...
<<ratechange,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
...
<<mx-problems,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
...
pdf(file='figures/graphics-mx-problems-1.pdf',height=8,width=8)
...
<<ex-shortrun,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
...
ggsave(filename='figures/graphics-ex-shortrun-1.pdf', plot=residuals_plot, height=5, width=8)
@
\end{figure}
...
<<proposals,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
...
ggsave(filename='figures/graphics-proposals-1.pdf', plot=proposal_plot, height=8, width=8)
@
\end{figure}
...
<<urod_react,out.width='\\linewidth',custom.plot=TRUE, fig.keep='all', fig.ext='pdf',echo=FALSE>>=
# create labels
...
urod_plot <- ggplot(urod.df, aes(year,urod)) + geom_point(aes(color=type),size=5) + scale_color_discrete(guide="none",labels=c("Technical Advisory Panel", "Trustees Report")) + scale_x_continuous("Year", breaks=seq(2000,2014,2), limits=c(1999,2018)) + scale_y_continuous("Ultimate Rate of Decline of Mortality (%)") + geom_segment_plus(data= arrow.df, aes( x=fromx, y=fromy, xend=tox, yend=toy),arrow = arrow(length = unit(0.25,"cm"),type="open", angle=20), lineend="round",alpha=0.5, size=0.5, shorten.start=0.015, shorten.end=0.015) + geom_text(data=urod.label, aes(x=x,y=y,color=type,label=label), size=4.5,family=ff) + geom_segment(aes(x=2017, y=0.6, xend=2017, yend=1.2),arrow = arrow(length = unit(0.25,"cm"),type="open", angle=20, ends="both"), lineend="round",alpha=0.5, size=0.5, color="steelblue") + geom_text(data= urod.highlow.label, aes(x,y,label=label), angle=0, size=4.5, color="steelblue",family=ff) + theme(text=element_text(size=15, color="black"))

ggsave(filename='figures/graphics-urod_react-1.pdf', plot=urod_plot, height=8, width=8)
@
\end{figure}
