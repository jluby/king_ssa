...
rates_of_change_df <- right_join(rates_of_change_df, bind)
...
urod_plot <- ggplot(urod.df, aes(year,urod)) + geom_point(aes(color=type),size=5) + scale_x_continuous("Year", breaks=seq(2000,2014,2), limits=c(1999,2018)) + scale_y_continuous("Ultimate Rate of Decline of Mortality (%)") + geom_text(data=urod.label, aes(x=x,y=y,color=type,label=label), size=4.5,family=ff) + geom_segment(aes(x=2017, y=0.6, xend=2017, yend=1.2),arrow = arrow(length = unit(0.25,"cm"),type="open", angle=20, ends="both"), lineend="round",alpha=0.5, size=0.5, color="steelblue") + geom_text(data= urod.highlow.label, aes(x,y,label=label), angle=0, size=4.5, color="steelblue",family=ff) + theme(text=element_text(size=15, color="black")) + guides(color=FALSE)
